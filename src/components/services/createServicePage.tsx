...
const CreateServicePage:React.FC<TheProps> = props => {
  ...
  if(getVolunteersQuery.loading || getFireTypesQuery.loading || getFireCausesQuery.loading || getFireClassesQuery.loading)
    return <Spinner/>
  ...
  return (
    <div>
      ...
      <div>
        ...
        <input value={input.description} onChange={event => {
            ...
            <Select field={`fire_type.id`} initialValue={undefined} >
              {
                getFireTypesQuery.data.fireTypes.map(fireType =>
                  <option value={fireType.id} key={fireType.id}>{fireType.name}</option>
                )
              }
            </Select>
            ...
            <Select field={`possible_cause.id`} initialValue={undefined} >
              {
                getFireCausesQuery.data.fireCauses.map(fireCause =>
                  <option value={fireCause.id} key={fireCause.id}>{fireCause.name}</option>
                )
              }
            </Select>
                  ...
                  <Select field={`fire_class[${i}].id`} /*initialValue={getFireClassesQuery.data.fireClasses}*/ >
                    {
                      getFireClassesQuery.data.fireClasses.map(fireClass =>
                        <option value={fireClass.id} key={fireClass.id}>{fireClass.name}</option>
                      )
                    }
                  </Select>
                  <br/>
